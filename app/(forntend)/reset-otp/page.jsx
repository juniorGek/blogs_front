...
import { PasswordReset, signupResendOtp } from '@/helpers/backend_helper';
...
const Page = () => {
    ...
    }, []); // Ce useEffect s'exécute seulement une fois après le rendu initial
    ...
    const onFinish = async (values) => {
        if (email) {
            const data = await PasswordReset({ email, otp: values.otp, action: "password_reset" });
            if (data?.error) {
                Error(data?.msg);
            } else {
                Success();
    ...
    const Success = () => {
        messageApi.open({
            type: 'success',
            content: i18n?.t('OTP Matched. Please Reset password.'),
        });
    };
        ...
        <Suspense fallback={<div>Loading...</div>}>
            ...
            <div className='dark:bg-BG_Color'>
                ...
                <div className=' w-4/5 md:w-3/5 mx-auto py-20'>
                    ...
                    <p className='paragraph_2 text-Font2_Color pb-4'>{i18n?.t('Your code is sent to this')} <span className=' text-Primary_Color'>{email}</span> {i18n?.t('email account.')}</p>
                    ...
                    <div>
                        <p className="dark:text-White_Color">
                            {time === 0 ? (
                                ...
                                }}>{i18n?.t('Resend OTP')}</button>
                            ) : (
                                <span>{i18n?.t('Resend in')} {time}{i18n?.t('s')}</span>
                            )}
                        </p>
                    </div>
                </div>
